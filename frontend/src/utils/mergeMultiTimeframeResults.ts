...
import { AnalysisResult } from './mapApiResponse';
...
export function mergeMultiTimeframeResults(
  results: Array<{ timeframe: 'M1' | 'M3' | 'M5'; result: LocalAnalysisResult }>
): AnalysisResult {
  if (results.length === 0) {
  ...
  const multiTimeframe: MultiTimeframeEntry[] = results.map(({ timeframe, result }) => ({
    timeframe,
    sinal: result.sinal,
    tendencia: result.tendencia,
    confianca: result.confianca,
